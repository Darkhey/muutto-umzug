...
if (!supabaseUrl || !serviceRole || !stripeSecret || !priceOneTime || !priceMonthly || !frontendUrl) {
...
serve(async (req) => {
  ...
  const success = body.success_url || `${frontendUrl}/premium?success=true`
  const cancel = body.cancel_url || `${frontendUrl}/premium?canceled=true`
  let session
  try {
